...
conditionalPanel(
  ...
  dateRangeInput("daterange", "Date Range:", format = "mm/dd/yy", start = "2020-01-22")
)
conditionalPanel(
  condition = "input.rangeornot == 'To Date'",
  dateInput("todate", "To Date:", format = "mm/dd/yy")
)
...
conditionalPanel(
  ...
  radioButtons("totalorpercent", "Display Totals or Percentages:",
             c("Total", "Percentage of Population"), selected = "Total")
)

conditionalPanel(
  condition = "input.casesordeaths == 'Deaths'",
  radioButtons("totalorpercentofcases", "Display Totals or Percentages:",
             c("Total", "Percentage of Cases", "Percentage of Population"), selected = "Total")
)
...
renderLeaflet({
  ...
  covid_deaths <- read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Deaths.csv")
  ...
  if (input$casesordeaths == "Cases"){
    if (input$totalorpercent == "Total"){
      map <- tm_shape(map_and_cases) + tm_polygons("Cases", id="country_name")
    } else {
      map <- tm_shape(map_and_cases) + tm_polygons("Percent of Population", id="country_name")
    }
  } else {
    if (input$totalorpercentofcases == "Total"){
      map <- tm_shape(map_and_deaths) + tm_polygons("Deaths", id="country_name")
    } else if (input$totalorpercentofcases == "Percentage of Population") {
      map <- tm_shape(map_and_deaths) + tm_polygons("Percent of Population", id="country_name")
    } else {
      map <- tm_shape(map_and_deaths) + tm_polygons("Percent of Cases", id="country_name")
    }
  }
